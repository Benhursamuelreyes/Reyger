...
def get_db_path():
    ...
    if __import__('sys').platform == 'win32':
        ...
        if getattr(sys_module, 'frozen', False):
            base_dir = __import__('os').path.dirname(sys_module.executable)
        else:
            base_dir = __import__('os').path.dirname(__import__('os').path.abspath(__file__))
    else:
        if getattr(__import__('sys'), 'frozen', False):
            base_dir = __import__('os').path.dirname(__import__('sys').executable)
        else:
            base_dir = __import__('os').path.dirname(__import__('os').path.abspath(__file__))
    
    return __import__('os').path.join(base_dir, "database.db")
